...
export default function CreatePost({ onAddPost }) {
  ...
  const handlePostSubmit = (e) => {
    ...
    if (onAddPost) {
      onAddPost({
        content: postText,
        type: postType,
        imageFile: postType === 'photo' ? imageFile : null,
        videoFile: postType === 'video' ? videoFile : null,
        title: (postType === 'event' || postType === 'article') ? title : null,
  ...
  return (
    <div className="_feed_inner_text_area _b_radious6 _padd_b24 _padd_t24 _padd_r24 _padd_l24 _mar_b16">
      ...
      {/* Dynamic Input Fields based on postType */}
      {postType === 'photo' && (
        <div style={{ marginTop: '15px', paddingLeft: '60px' }}>
          <div style={{
            ...
            />
            <div style={{ color: '#377DFF', fontWeight: '600', fontSize: '14px' }}>
              {imageFile ? `📸 Selected: ${imageFile.name}` : '📁 Click or Drag to upload Photo'}
            </div>
            <div style={{ color: '#666', fontSize: '12px', marginTop: '4px' }}>
              Accepts PNG, JPG, GIF (Max 10MB)
            </div>
          </div>
        </div>
      )}
      {postType === 'video' && (
        <div style={{ marginTop: '15px', paddingLeft: '60px' }}>
          <div style={{
            ...
            />
            <div style={{ color: '#47a062', fontWeight: '600', fontSize: '14px' }}>
              {videoFile ? `🎥 Selected: ${videoFile.name}` : '📁 Click or Drag to upload Video'}
            </div>
            <div style={{ color: '#666', fontSize: '12px', marginTop: '4px' }}>
              Accepts MP4, MOV, AVI, WEBM (Max 50MB)
            </div>
          </div>
        </div>
      )}
      {postType === 'event' && (
        <div style={{ marginTop: '15px', paddingLeft: '60px', display: 'flex', flexDirection: 'column', gap: '10px' }}>
          <input 
            type="text" 
            ...
            style={{ borderRadius: '8px', fontSize: '13px', padding: '10px 14px', border: '1px solid #e2e8f0' }}
          />
          <input 
            type="datetime-local" 
            className="form-control" 
            ...
            style={{ borderRadius: '8px', fontSize: '13px', padding: '10px 14px', border: '1px solid #e2e8f0', color: '#4a5568' }}
          />
        </div>
      )}
      {postType === 'article' && (
      ...
      <div className="_feed_inner_text_area_bottom">
        <div className="_feed_inner_text_area_item">
          <div 
            ...
            style={{ background: postType === 'photo' ? '#eef3ff' : 'none', borderRadius: '20px', transition: 'background 0.2s' }}
          >
            <button type="button" className="_feed_inner_text_area_bottom_photo_link" onClick={() => setPostType(prev => prev === 'photo' ? 'text' : 'photo')}> 
          ...
          <div 
            ...
            style={{ background: postType === 'video' ? '#eef3ff' : 'none', borderRadius: '20px', transition: 'background 0.2s' }}
          >
            <button type="button" className="_feed_inner_text_area_bottom_photo_link" onClick={() => setPostType(prev => prev === 'video' ? 'text' : 'video')}> 
              <span className="_feed_inner_text_area_bottom_photo_iamge _mar_img">
          ...
          <div 
            className="_feed_inner_text_area_bottom_event _feed_common"
            style={{ background: postType === 'event' ? '#eef3ff' : 'none', borderRadius: '20px', transition: 'background 0.2s' }}
          >
            <button type="button" className="_feed_inner_text_area_bottom_photo_link" onClick={() => setPostType(prev => prev === 'event' ? 'text' : 'event')}> 
              <span className="_feed_inner_text_area_bottom_photo_iamge _mar_img">
                <svg xmlns="http://www.w3.org/2000/svg" width="22" height="24" fill="none" viewBox="0 0 22 24">
          ...
          <div 
            className="_feed_inner_text_area_bottom_article _feed_common"
            style={{ background: postType === 'article' ? '#eef3ff' : 'none', borderRadius: '20px', transition: 'background 0.2s' }}
          >
            <button type="button" className="_feed_inner_text_area_bottom_photo_link" onClick={() => setPostType(prev => prev === 'article' ? 'text' : 'article')}> 
              <span className="_feed_inner_text_area_bottom_photo_iamge _mar_img">
        ...
        <div className="_feed_inner_text_area_btn" style={{ display: 'flex', alignItems: 'center' }}>
          <select 
